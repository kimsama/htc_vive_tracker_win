	...
	if (vt.InitializeVR(verbose)){
		...
		if (monitor_events){
			...
			if (vt.EventPolling()) {
				...
				if (vt.GetLastButtonPressedEnum("tracker_1")==vr::k_EButton_ApplicationMenu || vt.GetLastButtonPressedEnum("controller_1") == vr::k_EButton_ApplicationMenu) button_menu_pressed = true;
			}
		    }
		}
