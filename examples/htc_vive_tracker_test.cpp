...
void PrintAllDevicesInformation (CHtc_Vive_Tracker & vt)
{
  ...
  for (unsigned int i = 0; i < list_of_devices.size(); ++i)
  {
    double pose[3];
    double quat[4];
    if (vt.IsDeviceDetected(list_of_devices[i]))
    {
      ...
      std::cout<<list_of_devices[i]<<" not detected"<<std::endl;
  }
}
    ...
    if (monitor_events)
    {
        ...
        while (!button_menu_pressed)
        {
          if (vt.EventPolling()) 
          {
            ...
            if (vt.GetLastButtonPressedEnum("tracker_1")==vr::k_EButton_ApplicationMenu || 
                vt.GetLastButtonPressedEnum("controller_1") == vr::k_EButton_ApplicationMenu) 
